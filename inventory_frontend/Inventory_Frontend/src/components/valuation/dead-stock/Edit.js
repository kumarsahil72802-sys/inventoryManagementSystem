  ...
  useEffect(() => {
    if (editData) {
      setFormData({
        ...
        lastMovement: editData.lastMovementDate ? new Date(editData.lastMovementDate).toISOString().split('T')[0] : (editData.lastMovement || ''),
        costPrice: editData.costPrice || '',
        currentValue: editData.totalValue || editData.currentValue || '',
