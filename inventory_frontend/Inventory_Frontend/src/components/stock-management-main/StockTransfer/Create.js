...
const CreateStockTransfer = ({ onClose, onSave, items = [], warehouses = [], loading = false }) => {
  ...
  const addItem = () => {
    setTransferItems([...transferItems, { itemId: "", quantity: "", unit: "Pieces", costPrice: "" }]);
  };
  ...
  return (
    <Grid container spacing={2}>
      ...
      <Grid size={{ xs: 12, md: 6 }}>
        <FormControl fullWidth required>
          ...
          <Select name="toWarehouseId" value={formData.toWarehouseId} onChange={handleChange} label="To Warehouse">
            {Array.isArray(warehouses) && warehouses.filter(wh => wh.id !== formData.fromWarehouseId).map(wh => (
      ...
      <Grid size={{ xs: 12, md: 6 }}>
        <TextField fullWidth label="Reason" name="reason" value={formData.reason} onChange={handleChange} placeholder="Transfer reason" />
      </Grid>
      <Grid size={{ xs: 12, md: 6 }}>
        ...
        {transferItems.map((item, index) => (
          <Box key={index} sx={{ display: 'flex', gap: 1, mb: 1, alignItems: 'center' }}>
            ...
            <TextField sx={{ flex: 1 }} label="Qty" name="quantity" type="number" size="small"
              value={item.quantity} onChange={(e) => handleItemChange(index, e)} required />
      ...
      <Grid size={{ xs: 12 }}>
        <TextField fullWidth label="Notes" name="notes" value={formData.notes} onChange={handleChange}
          multiline rows={2} placeholder="Additional notes" />
      </Grid>
