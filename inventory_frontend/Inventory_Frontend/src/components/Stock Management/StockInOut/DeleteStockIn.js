...
const DeleteStockIn = ({ stockData, onClose, onDelete, loading = false }) => {
  if (!stockData) return null;
  return (
    <Grid container spacing={2}>
      <Grid size={{ xs: 12 }}>
        <Typography variant="body1">
          Are you sure you want to delete the <strong>Stock In</strong> record for <strong>{stockData.productName}</strong>? This action cannot be undone.
        </Typography>
      </Grid>
